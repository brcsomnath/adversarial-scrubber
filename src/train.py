...
def get_parser():
    ...
    parser.add_argument("--dataset", default="wizard", type=str, help="Dataset name.")
    parser.add_argument(
        "--MODEL",
    ...
    parser.add_argument(
        "--results_save_path",
        default="../results/wizard/",
        type=str,
        help="Save path of the dataset.",
    )
    parser.add_argument("--epochs", default=3, type=int, help="Number of epochs.")
    parser.add_argument("--delta_on", default=False, type=bool)
    parser.add_argument("--entropy_on", default=True, type=bool)
    parser.add_argument("--save_models", default=True, type=bool)
    parser.add_argument(
        "--lambda_1", default=10, type=int, help="lambda 1 value (int)."
    )
    parser.add_argument("--lambda_2", default=1, type=int, help="lambda 2 value (int).")
...
def eval_AdS(epoch, test_loader, bert_model, netS, D_bias, D_task, device):
    ...
    for step, (data, label, bias) in enumerate(test_loader):
        ...
        test_loader.set_description(
            f"task eval acc = {(task_acc.item()):.6f} bias eval acc = {(bias_acc.item()):.6f}"
        )
...
def generate_purged_dataset(dataset_loader, bert_model, netS, device):
    ...
    for data, label, bias in tqdm(dataset_loader):
        ...
        data_slice = [
            (data, label, bias) for data, label, bias in zip(purged_emb, label, bias)
        ]
...
def save_representations(
    args, bert_model, netS, train_dataloader, test_dataloader, device
):
...
def main():
    ...
    train_dataloader, test_dataloader = load(train_dataset, args.batch_size), load(
        test_dataset, args.batch_size
    )

    # Initialize models
    bert_model, netS, D_bias, D_task = initialize_models(
        args, Y_LABELS, Z_LABELS, device
    )
    ...
    optimD_bias = optim.AdamW(
        [{"params": D_bias.parameters()}], lr=2e-5, betas=(0.5, 0.999)
    )
    optimS = optim.AdamW(
        [
            {"params": netS.parameters()},
            {"params": D_task.parameters()},
